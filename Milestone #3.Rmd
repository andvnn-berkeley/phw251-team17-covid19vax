...
vax_latest <- vax_latest %>% 
  group_by(county) %>%
  mutate(countyfull = sum(persons_fully_vaccinated, na.rm = T)/sum(age12_plus_population),
         countypartial = sum(persons_partially_vaccinated, na.rm = T)/sum(age12_plus_population)) %>% 
  ungroup() %>% 
  mutate(percent_full = persons_fully_vaccinated/age12_plus_population,
         percent_partial = persons_partially_vaccinated/age12_plus_population) %>% 
